...
def start_bot():
    ...
    @bot.message_handler(commands=['start'])#СЛИТО В @END_RAID
    def start(message):
        conn = sqlite3.connect('base_pyramid.sqlite')#СЛИТО В @END_RAID
        cursor = conn.cursor()
        ...
        if len(row) == 0:
            ...
            if len(row) == 0:
                ...
                cursor.execute(
                    f'INSERT INTO users VALUES ("{message.chat.id}", "{message.from_user.username}", "{datetime.datetime.now()}", "{who_invite}", "{referral_code}", "no")')
                conn.commit()
